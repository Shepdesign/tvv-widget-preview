...
/* Compact 2-up gate: Name | Email, Phone (full), Rating | Dependents */
.tvv__grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0 12px; }
.tvv__grid .tvv__field { min-width: 0; }
.tvv__field--full { grid-column: 1 / -1; }

/* TCPA consent \u2014 checkbox + fine-print notice */
.tvv__consent { display: flex; gap: 10px; align-items: flex-start; margin: 12px 0 0; cursor: pointer; }
.tvv__check { flex: 0 0 auto; width: 18px; height: 18px; margin: 1px 0 0; accent-color: var(--tvv-gold); cursor: pointer; }
.tvv__consent-text { font-size: 0.72rem; line-height: 1.5; color: var(--tvv-text-soft); }
...
.tvv__error:empty { display: none; margin: 0; }
...
`;function u(o){let t=typeof o=="number"?o:parseFloat(o);return!isFinite(t)||t<0?0:t}function g(o,t){let e=u(o.non_taxable_income),r=u(o.other_income),n=u(o.monthly_debts),i=e*(1+t.gross_up_rate),s=i+r,a=s*t.dti_cap-n,v=i-e,m=0;if(a>0){let f=t.interest_rate/12,h=t.term_years*12;m=f===0?a*h:a*(1-Math.pow(1+f,-h))/f}return{grossed_up:i,qualifying_income:s,max_housing_payment:Math.max(a,0),boost:v,buying_power:Math.round(m)}}function l(o){return"$"+Math.round(o).toLocaleString("en-US")}function p(o){return String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var w=`<svg class="tvv__mark" width="70" height="70" viewBox="0 0 120 120" role="img" aria-label="True Vet Value">
<defs>
...
<use href="#tvvBeads6"/>
</svg>`,k='<div class="tvv__header">'+w+'<div class="tvv__brand"><span class="tvv__wordmark">TRUE VET <span class="tvv__gold">VALUE</span></span><span class="tvv__eyebrow">Know your worth</span></div></div>',x="By checking this box, I agree that True Vet Value and its lending partners may contact me at the phone number and email above about my results and mortgage options \u2014 including by automated dialing system, prerecorded or artificial voice, and text message. Consent is not a condition of any purchase. Message and data rates may apply; reply STOP to opt out.",d=class{constructor(t,e){this.root=t,this.config=e,this.state="live",this.inputs={non_taxable_income:"4194.54",other_income:"1500",monthly_debts:"400"},this.snapshot=g(this.inputs,e.defaults),this.captured=!1,this.error=""}render(){this.root.innerHTML=this.template(),this.bind()}template(){return'<div class="tvv">'+k+this.formTemplate()+this.teaserTemplate()+`<div class="tvv__panel" data-tvv-panel>${this.panelTemplate()}</div><p class="tvv__disclaimer">${p(this.config.disclaimer)}</p></div>`}fieldTemplate(t,e,r){return`<div class="tvv__field"><label class="tvv__label" for="${t}">${p(e)}</label><div class="tvv__inputwrap"><span class="tvv__prefix">$</span><input class="tvv__input" id="${t}" type="number" min="0" step="1" inputmode="decimal" value="${p(r)}" data-tvv-amount></div></div>`}formTemplate(){return'<div class="tvv__form"><p class="tvv__title">Estimate your VA buying power</p>'+this.fieldTemplate("tvv-non-taxable","Non-taxable monthly income",this.inputs.non_taxable_income)+this.fieldTemplate("tvv-other","Other monthly income",this.inputs.other_income)+this.fieldTemplate("tvv-debts","Total monthly debt payments",this.inputs.monthly_debts)+"</div>"}teaserTemplate(){let t=this.snapshot;return`<div class="tvv__teaser"><span class="tvv__teaser-lbl">Grossed-up monthly income</span><strong class="tvv__teaser-val" id="tvv-grossed">${l(t.grossed_up)}</strong><span class="tvv__teaser-sub"><span id="tvv-boost">+${l(t.boost)}</span> Monthly boost from grossing up</span></div>`}ratingOptions(){let t='<option value="">Select rating\u2026</option>';for(let e=0;e<=100;e+=10)t+=`<option value="${e}">${e}%</option>`;return t}panelTemplate(){return this.state==="revealed"?this.revealInner():this.gateInner()}gateInner(){return`<div class="tvv__gate"><p class="tvv__gate-cta">See the home price you may qualify for.</p><div class="tvv__grid"><div class="tvv__field"><label class="tvv__label" for="tvv-name">Name</label><input class="tvv__input" id="tvv-name" type="text" autocomplete="name" value=""></div><div class="tvv__field"><label class="tvv__label" for="tvv-email">Email</label><input class="tvv__input" id="tvv-email" type="email" autocomplete="email" value=""></div><div class="tvv__field tvv__field--full"><label class="tvv__label" for="tvv-phone">Phone</label><input class="tvv__input" id="tvv-phone" type="tel" autocomplete="tel" inputmode="tel" value=""></div><div class="tvv__field"><label class="tvv__label" for="tvv-rating">Current VA rating</label><select class="tvv__input tvv__select" id="tvv-rating">${this.ratingOptions()}</select></div><div class="tvv__field"><label class="tvv__label" for="tvv-dependents">Dependents</label><input class="tvv__input" id="tvv-dependents" type="number" min="0" step="1" inputmode="numeric" value=""></div></div><label class="tvv__consent"><input type="checkbox" id="tvv-consent" class="tvv__check"><span class="tvv__consent-text">${p(x)}</span></label><input class="tvv__hp" id="tvv-company" type="text" tabindex="-1" autocomplete="off" aria-hidden="true" value=""><p class="tvv__error" data-tvv-error></p><button type="button" class="tvv__btn" data-tvv-reveal>Reveal my buying power</button></div>`}revealInner(){let t=this.snapshot;return t.buying_power<=0?'<div class="tvv__reveal tvv__reveal--soft"><p class="tvv__result">Your current debts exceed the housing budget for this estimate. Lowering monthly debt can unlock buying power.</p></div>':`<div class="tvv__reveal"><span class="tvv__reveal-lbl">Estimated home price you may qualify for</span><strong class="tvv__reveal-val">${l(t.buying_power)}</strong><p class="tvv__next">A specialist can help you take the next step.</p></div>`}field(t){let e=this.root.querySelector("#"+t);return e?e.value:""}readInputs(){return{non_taxable_income:this.field("tvv-non-taxable"),other_income:this.field("tvv-other"),monthly_debts:this.field("tvv-debts")}}recalc(){this.inputs=this.readInputs(),this.snapshot=g(this.inputs,this.config.defaults);let t=this.root.querySelector("#tvv-grossed"),e=this.root.querySelector("#tvv-boost");if(t&&(t.textContent=l(this.snapshot.grossed_up)),e&&(e.textContent="+"+l(this.snapshot.boost)),this.state==="revealed"){let r=this.root.querySelector("[data-tvv-panel]");r&&(r.innerHTML=this.revealInner())}}showError(t){this.error=t;let e=this.root.querySelector("[data-tvv-error]");e&&(e.textContent=t)}onReveal(){let t=this.field("tvv-name"),e=this.field("tvv-email"),r=this.field("tvv-phone"),n=this.root.querySelector("#tvv-consent"),i=!!(n&&n.checked);if(!t||!e||!r)return this.showError("Please enter your name, email, and phone number."),Promise.resolve();if(!i)return this.showError("Please agree to be contacted to continue."),Promise.resolve();let s=Object.assign({name:t,email:e,phone:r,consent:i,consent_text:x,current_rating:this.field("tvv-rating"),dependents:this.field("tvv-dependents"),company:this.field("tvv-company"),source_url:typeof location!="undefined"?location.href:""},this.inputs,this.snapshot);return fetch(this.config.functionUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)}).then(a=>{if(!a.ok)throw new Error("request failed");return a.json()}).then(()=>{this.captured=!0,this.state="revealed",this.error="",this.render()}).catch(()=>{this.showError("Something went wrong. Please try again.")})}bind(){this.root.querySelectorAll("[data-tvv-amount]").forEach(e=>{e.addEventListener("input",()=>this.recalc())});let t=this.root.querySelector("[data-tvv-reveal]");t&&t.addEventListener("click",()=>this.onReveal())}};function E(o){let t=o&&o.dataset||{},e=Object.assign({},c.defaults);return t.grossUpRate!==void 0&&(e.gross_up_rate=parseFloat(t.grossUpRate)),t.dtiCap!==void 0&&(e.dti_cap=parseFloat(t.dtiCap)),t.interestRate!==void 0&&(e.interest_rate=parseFloat(t.interestRate)),t.termYears!==void 0&&(e.term_years=parseInt(t.termYears,10)),{mode:t.tvvMode==="popup"?"popup":"inline",functionUrl:t.functionUrl||c.functionUrl,disclaimer:c.disclaimer,defaults:e}}function y(o){let t=document.createElement(o),e=t.attachShadow({mode:"open"}),r=document.createElement("style");r.textContent=_,e.appendChild(r);let n=document.createElement("div");return e.appendChild(n),{host:t,shadow:e,root:n}}function b(o,t){let{host:e,root:r}=y("div");return e.className="tvv-host",o.tagName==="SCRIPT"?o.parentNode.insertBefore(e,o.nextSibling):o.appendChild(e),new d(r,t).render(),e}function C(o){let{host:t,root:e}=y("div");t.className="tvv-host tvv-host--popup",document.body.appendChild(t),e.innerHTML='<button type="button" class="tvv__launcher" data-tvv-open>Estimate your VA buying power</button><div class="tvv__modal" data-tvv-modal hidden><div class="tvv__backdrop" data-tvv-close></div><div class="tvv__dialog"><button type="button" class="tvv__close" data-tvv-close aria-label="Close">&times;</button><div data-tvv-widget-root></div></div></div>';let r=e.querySelector("[data-tvv-modal]"),n=e.querySelector("[data-tvv-widget-root]"),i=null;function s(){i||(i=new d(n,o),i.render()),r.hidden=!1}function a(){r.hidden=!0}return e.querySelector("[data-tvv-open]").addEventListener("click",s),e.querySelectorAll("[data-tvv-close]").forEach(v=>v.addEventListener("click",a)),document.addEventListener("keydown",v=>{v.key==="Escape"&&a()}),{host:t,open:s,close:a}}function F(o){let t=E(o),e=document.querySelector("[data-tvv-mount]");t.mode==="popup"?C(t):e?b(e,t):o&&b(o,t)}typeof document!="undefined"&&document.currentScript&&F(document.currentScript);})();
